...
export default class MapBuilder extends React.Component {
  ...
  boundMapByWalk() {
    ...
    if (this.state.markers.getLength()) {
      for (let i = 0, len = this.state.markers.getLength(); i < len; i++) {
  ...
  buildMarker(options) {
    ...
    google.maps.event.addListener(marker, 'click', ev => this.showInfoWindow(marker));
  ...
  buildRoute(routeArray) {
    ...
    google.maps.event.addListener(poly, 'mousedown', ev => this.state.infowindow.setMap(null));
  ...
  toggleAddPoint() {
    ...
    this.setState({markers: markers, mode: {}}, () => {
      this.syncState();
