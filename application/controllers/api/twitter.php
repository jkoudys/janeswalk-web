...
use Controller;
use Qaribou\Collections\ImmArray;
...
class Twitter extends Controller
{
     ...
     */
    protected function getSearch($query, $coords = null): ImmArray
    {
        $ch = curl_init();
        curl_setopt_array(
            $ch,
        ...
        $res = ImmArray::fromArray((array) json_decode(curl_exec($ch), true)['statuses']);

        // Filter only the geocoded tweets, and map to our lat/lng format
        return $res->filter(function($status) {
            return $status['geo']['type'] === 'Point';
        })->map(function($status) {
            return [
