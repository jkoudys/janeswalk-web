...
class ParentPageOwnerPermissionAccessEntity extends PermissionAccessEntity {
    ...
    if (is_object($c) && ($c instanceof Page)) {
      $pcID = $c->getCollectionParentID() ?: $c->getCollectionID(); // Check against this page, if no parent set
	...
	public function getAccessEntityTypeLinkHTML() {
		$html = '<a href="javascript:void(0)" onclick="ccm_choosePermissionAccessEntityParentPageOwner()">' . tc('PermissionAccessEntityTypeName', 'Parent Page Owner') . '</a>';
		return $html;		
	}
	...
	public static function getAccessEntitiesForUser($user) {
		...
		if ($user->isRegistered()) { 
			$pae = ParentPageOwnerPermissionAccessEntity::getOrCreate();
	...
	public static function getOrCreate() {
		$db = Loader::db();
		$petID = $db->GetOne('select petID from PermissionAccessEntityTypes where petHandle = \'parent_page_owner\'');
	...
	public function load() {
		$this->label = t('Parent Page Owner');
	}
